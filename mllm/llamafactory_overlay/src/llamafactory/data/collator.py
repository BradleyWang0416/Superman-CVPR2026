...
class MultiModalDataCollatorForSeq2Seq(DataCollatorForSeq2Seq):
    ...
    def __call__(self, features: list[dict[str, Any]]) -> dict[str, "torch.Tensor"]:
        ...
        for feature in features:
            ...
            skeletons = feature.pop("skeletons", None) or []
            batch_images.extend(images)
            batch_videos.extend(videos)
            batch_audios.extend(audios)
            batch_skeletons.extend(skeletons)
            batch_imglens.append(len(images))
            batch_vidlens.append(len(videos))
            batch_audlens.append(len(audios))
            batch_skelens.append(len(skeletons))
        ...
        if self.get_rope_func is not None:
            ...
            # transformers >= 5 made mm_token_type_ids a required argument of
            # Qwen3VLModel.get_rope_index. It is derived from input_ids alone
            # (0 = text, 1 = image, 2 = video, 3 = audio); skeleton tokens are ordinary
            # vocabulary entries and correctly fall into the text class.
            if "mm_token_type_ids" in inspect.signature(self.get_rope_func).parameters:
                rope_index_kwargs["mm_token_type_ids"] = torch.tensor(
                    self.processor.create_mm_token_type_ids(features["input_ids"].tolist()),
                    dtype=torch.int,
