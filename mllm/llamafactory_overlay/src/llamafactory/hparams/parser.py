...
def read_args(args: Optional[Union[dict[str, Any], list[str]]] = None) -> Union[dict[str, Any], list[str]]:
    ...
    if sys.argv[1].endswith(".yaml") or sys.argv[1].endswith(".yml"):
        ...
        try:
            OmegaConf.register_new_resolver("config_dir", lambda path: str(config_path.parent / path))
        except ValueError:
            pass
        override_config = OmegaConf.from_cli(sys.argv[2:])
        dict_config = OmegaConf.load(config_path)
        dict_config["config_dir"] = str(config_path.parent)
...
def get_train_args(args: Optional[Union[dict[str, Any], list[str]]] = None) -> _TRAIN_CLS:
    ...
    if (
        ...
        and not (training_args.overwrite_output_dir if "overwrite_output_dir" in training_args.__dataclass_fields__ else False)
