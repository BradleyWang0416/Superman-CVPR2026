...
class DataArguments:
    ...
    def __post_init__(self):
        ...
        if self.dataset_eval_range is not None:
            import ast
            if isinstance(self.dataset_eval_range, dict):
                for key in self.dataset_eval_range:
                    val = self.dataset_eval_range[key]
                    if 'slice' in val:
                        self.dataset_eval_range[key] = eval(val)
                        assert self.dataset_eval_range[key].__class__ == slice
                    elif 'range' in val:
                        self.dataset_eval_range[key] = eval(val)
                        assert self.dataset_eval_range[key].__class__ == range
                    else:
                        self.dataset_eval_range[key] = ast.literal_eval(val)
                        assert isinstance(self.dataset_eval_range[key], list)
                        assert all(isinstance(eval_range_item, int) for eval_range_item in self.dataset_eval_range[key])

            elif isinstance(self.dataset_eval_range, str):
                if 'slice' in self.dataset_eval_range:
                    self.dataset_eval_range = eval(self.dataset_eval_range)
                    assert self.dataset_eval_range.__class__ == slice
                elif 'range' in self.dataset_eval_range:
                    self.dataset_eval_range = eval(self.dataset_eval_range)
                    assert self.dataset_eval_range.__class__ == range
                else:
                    self.dataset_eval_range = ast.literal_eval(self.dataset_eval_range)
                    assert isinstance(self.dataset_eval_range, list)
                    assert all(isinstance(eval_range_item, int) for eval_range_item in self.dataset_eval_range)
