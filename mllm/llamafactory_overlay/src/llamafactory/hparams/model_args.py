...
class BaseModelArguments:
    ...
    def __post_init__(self):
        ...
        if self.vqvae_config is None:
            raise ValueError("`vqvae_config` is required for the released Superman model.")

        if not os.path.exists(self.vqvae_config) and len(sys.argv) > 1 and (sys.argv[1].endswith(".yaml") or sys.argv[1].endswith(".yml")):
            config_dir = os.path.dirname(os.path.abspath(sys.argv[1]))
            candidate_path = os.path.join(config_dir, self.vqvae_config)
            if os.path.exists(candidate_path):
                self.vqvae_config = candidate_path
