...
def _setup_lora_tuning(
    ...
    if is_trainable and adapter_to_resume is None:  # create new lora weights while training
        if len(finetuning_args.lora_target) == 1 and finetuning_args.lora_target[0] == "all":
            ...
            all_linear_modules_tmp = find_all_linear_modules(model, finetuning_args.freeze_vision_tower)
            target_modules = finetuning_args.lora_target
            excluded_modules = [module for module in all_linear_modules_tmp if module not in target_modules]
            logger.info_rank0(f"Linear modules excluded from LoRA targets: {excluded_modules}")
